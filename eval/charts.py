...
INK = "#7d8590"        # axis labels, readable on light and dark
FAINT = "#7d8590"      # grid, used at low opacity
K3 = "#f85149"
BLUE = "#58a6ff"
GREEN = "#3fb950"
GOLD = "#d29922"
COLOR = {
    "K3-REAP80-hr-code": K3,
    "gpt-oss-120b": BLUE,
    "gemma4-31b": GREEN,
    "gpt-oss-20b": GOLD,
}
...
def text(x, y, s, size=12, fill=INK, anchor="start", weight="400", opacity=1.0):
...
def line(x1, y1, x2, y2, stroke=FAINT, width=1, opacity=0.25, dash=None):
...
def fig_size_vs_score() -> str:
    ...
    b = [text(L, 24, "Croatian comprehension against model size", 14, INK, weight="600")]
...
def fig_response_split() -> str:
    ...
    b = [text(20, 26, "Every score splits in two: did it answer, was it right", 14,
         ...
         text(20, 45, "K3 is the only model here that fails by not answering at all",
              11, INK, opacity=0.8)]
    for i, (lab, col, op) in enumerate([("correct", GREEN, 1.0),
                                        ("answered, wrong", INK, 0.45),
                                        ("no answer", K3, 1.0)]):
        x = 300 + i * 150
    ...
    for m in A.MODELS:
        ...
        for task, tname in tasks:
            ...
            for frac, col, op in ((correct, GREEN, 1.0),
                                  (answered_wrong, INK, 0.45),
                                  (silent, K3, 1.0)):
                b.append(rect(x, ty, frac * full, bar_h, col, op))
                x += frac * full
            b.append(text(L - 10, ty + 13, tname, 11, INK, "end", opacity=0.85))
            b.append(text(L + 7, ty + 13, f"{correct*100:.0f}%", 10.5, "#ffffff",
                          weight="700"))
...
def fig_language_gap() -> str:
    ...
    b = [text(20, 26, "Croatian minus English, on identical passages", 14, INK,
         ...
         text(20, 45, "K3 answers far more in Croatian. It is not more accurate in it.",
              11, INK, opacity=0.8)]
    b.append(rect(W - R - 118, 18, 10, 10, K3, 1.0))
    b.append(text(W - R - 103, 27, "response rate", 11, INK, opacity=0.85))
    b.append(rect(W - R - 118, 38, 10, 10, INK, 0.45))
    ...
    for m in A.MODELS:
        ...
        b.append(text(L - 14, y + band / 2 + 4, SHORT[m], 12, COLOR[m], "end",
                      weight="600"))
        for k, (d, col, op) in enumerate([(d_resp, K3, 1.0), (d_acc, INK, 0.45)]):
